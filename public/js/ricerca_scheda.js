fetch("ricerca_scheda/inizializza").then(onResponse, onError).then(displayEsercizi);
...
function displayEsercizi(json_lista_esercizi){
    ...
    function ricerca_scheda(event){
        ...
        if(form.ricerca.value.length==0){
            ...
            fetch('ricerca_scheda/utente/' + encodeURIComponent(username)).then(handler, onError).then(onSearch_scheda);
        }
